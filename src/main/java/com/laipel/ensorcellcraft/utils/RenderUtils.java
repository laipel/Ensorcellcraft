...
public class RenderUtils {
    ...
    public static void drawStreak(IStreakable<? extends Entity> streakable, float pTicks, PoseStack poseStack, MultiBufferSource bufferSourceList) {
        ...
        IStreakable.Streak streak = streakable.getStreak();
        IStreakable.StreakBuffer streakBuffer = streakable.getStreakBuffer();
        ...
        boolean anotherTick = streakBuffer.getTime() != time;
        ...
        for (int i = 0; streakBuffer.contains(i) && i <= segments + 1; i++)
            savedPoss.add(streakBuffer.get(i));

        if (time % streak.getSegmentsLife() == 0 && anotherTick) {
            streakBuffer.setTime(time);
            if (savedPoss.size() > segments) {
        ...
        Vec3[][] crossVecs = new Vec3[prevPoses.size()][3];
        ...
        for (int i = 0; i < prevPoses.size(); i++) {
            ...
            if (i == crossVecs.length - 1 || crossVecs[i + 1][0] == null) {
                ...
                TesselatorUtils.drawQuadGradient(tes, matrix4f, pos13.x, pos13.y, pos13.z, pos23.x, pos23.y,
                        pos23.z, pos21.x, pos21.y, pos21.z, pos11.x, pos11.y, pos11.z, c1, c2);
